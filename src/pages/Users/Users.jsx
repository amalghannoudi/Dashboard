  ...
  async  function deleteUser (id) {
    let result =await  fetch ("http://127.0.0.1:8000/api/deleteUser/"+id,{
      ...
      async function fetchUser () {
        let result = await fetch(`http://127.0.0.1:8000/api/getUsers`);
        result = await result.json();
        console.log(result);
...
<br></br>
    <div class="products-area-wrapper tableView">
      ...
      {user.map(user => (
      <div class="products-row" key={user.id}>
        <button class="cell-more-button">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-more-vertical"><circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" r="1"/><circle cx="12" cy="19" r="1"/></svg>
        </button>
       
        <div class="product-cell sales">{user.id}</div>

             <div class="product-cell sales">{user.name}</div>

            
             <div  class="product-cell sales" >{user.email}</div>

           
             <div class="actions">
             <UpdateUser props={user.id}  onUpdateSuccess={handleUpdate} />
